...
contract ForkMintableTokenV3 is ERC20 {
    constructor() ERC20("Fork Liquidity Token", "FLT") {
...
/// @notice Pinned, read-only Base Mainnet fork validation of the canonical
/// Uniswap V3 launch-reservation boundary. All writes are ephemeral fork state.
contract BaseMainnetUniswapV3ForkTest is Test {
    uint256 private constant BASE_MAINNET_CHAIN_ID = 8_453;
    uint256 private constant PINNED_BLOCK = 48_887_205;
    uint24 private constant POOL_FEE = 10_000;
    int24 private constant TICK_SPACING = 200;
    uint256 private constant Q192 = 1 << 192;

    // Official Uniswap V3 Base Mainnet deployment addresses. These are
    ...
    address private constant NONFUNGIBLE_POSITION_MANAGER = 0x03a520b32C04BF3bEEf7BEb72E919cf822Ed34f1;
    ...
    function testStrictClassifierRejectsCanonicalPoolWithNonzeroLiquidity() public {
        GraduationManagerV3 manager = new GraduationManagerV3(UNISWAP_V3_FACTORY, WETH);
        ForkMintableTokenV3 token = new ForkMintableTokenV3();
        address pool = canonicalFactory.createPool(address(token), WETH, POOL_FEE);
        IUniswapV3PoolMinimal(pool).initialize(manager.expectedSqrtPriceX96(address(token)));

        vm.deal(address(this), 4 ether);
        IWETH9Fork(WETH).deposit{value: 3 ether}();
        assertTrue(token.approve(NONFUNGIBLE_POSITION_MANAGER, type(uint256).max));
        assertTrue(IERC20(WETH).approve(NONFUNGIBLE_POSITION_MANAGER, type(uint256).max));
        INonfungiblePositionManagerV3.MintParams memory params;
        (params.token0, params.token1) = address(token) < WETH ? (address(token), WETH) : (WETH, address(token));
        params.fee = POOL_FEE;
        params.tickLower = -887_200;
        params.tickUpper = 887_200;
        params.amount0Desired = params.token0 == address(token) ? 200_000_000 ether : 3 ether;
        params.amount1Desired = params.token0 == address(token) ? 3 ether : 200_000_000 ether;
        params.recipient = address(this);
        params.deadline = block.timestamp;
        (, uint128 positionLiquidity,,) = INonfungiblePositionManagerV3(NONFUNGIBLE_POSITION_MANAGER).mint(params);
        assertGt(positionLiquidity, 0);
        assertGt(IUniswapV3PoolMinimal(pool).liquidity(), 0);

        (IGraduationManagerV3.PoolCandidateState state, address classifiedPool) =
            manager.classifyPoolCandidate(address(token));
        assertEq(uint256(state), uint256(IGraduationManagerV3.PoolCandidateState.NonzeroLiquidity));
        assertEq(classifiedPool, pool);
    }

    function testCurrentEconomicsAndFeeDesignBPolicyRemainExact() public {
        (FeeManagerV3 fees, GraduationManagerV3 manager, ZonkFactoryV3 factory,,) = _deployConcreteBoundStack();
        vm.prank(CREATOR);
        (address token, address curveAddress) =
            factory.createToken("Mainnet Economics", "MNE", keccak256("mainnet-economics"));
        ZonkCurveV3 curve = ZonkCurveV3(payable(curveAddress));
        assertEq(ZonkTokenV3(token).totalSupply(), 1_000_000_000 ether);
        ...
        assertEq(curve.TOTAL_FEE_BPS(), 100);
        assertEq(curve.FEE_DENOMINATOR(), 10_000);
        assertEq(curve.feePolicyHash(), keccak256("zonk-fee-design-b-v3"));
        assertEq(fees.feePolicyHash(), keccak256("zonk-fee-design-b-v3"));
        assertEq(manager.protocolVersionHash(), keccak256("endpoint-cp-v3"));

        ZonkCurveV3.FeeSplit memory split = curve.splitFee(10_000);
        assertEq(split.totalFee, 100);
        assertEq(split.creatorFee, 35);
        assertEq(split.protocolFee, 30);
        assertEq(split.communityFee, 20);
        assertEq(split.traderRewardsFee, 15);
    }
    ...
    function testConcreteGraduationSettlementCustodyFeesAndThirdPartyPosition() public {
        ...
        ConcreteSettlement memory settlement = _launchAndGraduateConcrete(fees, manager, factory, deployer);
        ...
        private
    {
        ...
        uint256 total0Before = vault.totalLPFeesAccrued(settlement.token0);
        uint256 total1Before = vault.totalLPFeesAccrued(settlement.token1);
        INonfungiblePositionManagerV3.CollectParams memory params = INonfungiblePositionManagerV3.CollectParams({
            tokenId: thirdPartyId, recipient: THIRD_PARTY, amount0Max: type(uint128).max, amount1Max: type(uint128).max
        });
        vm.prank(THIRD_PARTY);
        (uint256 thirdPartyCollected0, uint256 thirdPartyCollected1) =
            INonfungiblePositionManagerV3(NONFUNGIBLE_POSITION_MANAGER).collect(params);
        assertTrue(thirdPartyCollected0 != 0 || thirdPartyCollected1 != 0);
        assertEq(vault.totalLPFeesAccrued(settlement.token0), total0Before);
        assertEq(vault.totalLPFeesAccrued(settlement.token1), total1Before);
    }
        ...
        )
    {
        ...
        assertEq(fees.ecosystemBootstrapAuthority(), address(0));
        assertEq(manager.factoryBootstrapAuthority(), address(0));
        assertEq(manager.dependencyBootstrapAuthority(), address(0));
        assertEq(community.lpFeeVaultBootstrapAuthority(), address(0));
        assertEq(rewards.bootstrapAuthority(), address(0));
        assertEq(vault.custodianDeployerBootstrapAuthority(), address(0));
        assertTrue(manager.settlementExecutor() != address(0));
    ...
    function _launchAndGraduateConcrete(
        ...
        uint256 totalFee = _buyToExactEndpoint(curve);
        ...
        _assertConcreteSettlement(fees, manager, curve, settlement, totalFee);
    }

    function _buyToExactEndpoint(ZonkCurveV3 curve) private returns (uint256 totalFee) {
        uint256 submittedGross = EndpointConstantsV3.EXACT_GRADUATION_GROSS + 1 ether;
        ZonkCurveV3.BuyQuote memory quote = curve.quoteBuy(submittedGross);
        assertEq(quote.acceptedGross, EndpointConstantsV3.EXACT_GRADUATION_GROSS);
        assertEq(quote.refund, 1 ether);
        assertEq(quote.netCurveInput, 3 ether);
        assertEq(quote.tokensOut, 800_000_000 ether);
        assertEq(quote.totalFee, quote.creatorFee + quote.protocolFee + quote.communityFee + quote.traderRewardsFee);
        assertEq(quote.creatorFee, quote.totalFee * 35 / 100);
        assertEq(quote.communityFee, quote.totalFee * 20 / 100);
        assertEq(quote.traderRewardsFee, quote.totalFee * 15 / 100);
        assertEq(quote.protocolFee, quote.totalFee - quote.creatorFee - quote.communityFee - quote.traderRewardsFee);
        vm.deal(BUYER, submittedGross);
        uint256 buyerEthBefore = BUYER.balance;
        vm.prank(BUYER);
        curve.buy{value: submittedGross}(quote.tokensOut, block.timestamp + 1);
        assertEq(buyerEthBefore - BUYER.balance, quote.acceptedGross);
        return quote.totalFee;
    }

    function _assertConcreteSettlement(
        FeeManagerV3 fees,
        GraduationManagerV3 manager,
        ZonkCurveV3 curve,
        ConcreteSettlement memory settlement,
        uint256 totalFee
    ) private {
        uint256 tokenId = settlement.custodian.boundTokenId();
        assertTrue(manager.settled(settlement.token));
        assertTrue(curve.graduated());
        assertEq(curve.soldSupply(), 800_000_000 ether);
        assertEq(curve.graduationEthForwarded(), 3 ether);
        assertEq(IERC20(settlement.token).balanceOf(BUYER), 800_000_000 ether);
        assertEq(IERC20(settlement.token).balanceOf(address(curve)), 0);
        assertEq(address(curve).balance, 1 ether, "only the deliberately forced ETH remains");
        assertEq(fees.totalLiabilities(), totalFee);
        vm.expectRevert();
        curve.quoteBuy(1);
        ...
        uint256 expectedTokenUsed = settlement.token0 == settlement.token
            ? 199_999_999_999_999_999_999_999_977
            : 199_999_999_999_999_999_999_999_968;
        ...
        uint256 expectedTokenResidual = 200_000_000 ether - expectedTokenUsed;
        assertEq(expectedTokenUsed + expectedTokenResidual, 200_000_000 ether);
        assertEq(settlement.residualEscrow.depositedResidual(settlement.token), expectedTokenResidual);
        assertEq(settlement.residualEscrow.depositedResidual(WETH), 1_332);
        assertEq(expectedWethUsed + settlement.residualEscrow.depositedResidual(WETH), 3 ether);
        assertEq(IERC20(settlement.token).balanceOf(address(settlement.residualEscrow)), expectedTokenResidual);
        ...
        assertEq(IERC20(settlement.token).balanceOf(manager.settlementExecutor()), 0);
        assertEq(IERC20(WETH).balanceOf(manager.settlementExecutor()), 0);
        assertEq(manager.settlementExecutor().balance, 0);
        ...
        vm.expectRevert();
        settlement.custodian.bindPosition(tokenId);
    }
