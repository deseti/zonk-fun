...
if [ "$expected" -ne 10 ]; then
  echo "migration set must contain exactly 001 through 009" >&2
  exit 1
fi

psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -c "
  DO \$\$ BEGIN
    IF (SELECT array_agg(version ORDER BY version) FROM schema_migrations) IS DISTINCT FROM ARRAY[1,2,3,4,5,6,7,8,9] THEN
      RAISE EXCEPTION 'schema_migrations must contain exactly versions 001 through 009';
    END IF;
  END \$\$" >/dev/null

echo "Database migrations 001 -> 002 -> 003 -> 004 -> 005 -> 006 -> 007 -> 008 -> 009 complete."
