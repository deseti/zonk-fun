      ...
      <div className="market-container">
        <div className="grid min-h-12 grid-cols-[auto_1fr_auto] items-center gap-3 md:min-h-16">
          <Link href="/" className="flex min-h-11 items-center gap-2 rounded-lg text-lg font-semibold tracking-[-0.04em] text-white md:text-xl" onClick={() => { setOpen(false); setSearchOpen(false); }} aria-label="Zonk.fun home">
            <span className="flex h-8 w-8 items-center justify-center overflow-hidden rounded-lg border border-cyan-300/20 bg-[#07111c]" data-logo-source="/favicon.ico"><Image src="/favicon.ico" alt="" width={32} height={21} priority unoptimized /></span>
          ...
          <div className="hidden items-center justify-end gap-2 xl:flex">
            ...
            <div className="hidden min-w-0 xl:flex"><WalletStatus /></div>
          </div>
          <div className="flex items-center justify-end gap-2 md:hidden">
            <WalletStatus compact short />
          </div>
        ...
        {open && <div id="mobile-navigation" className="hidden border-t border-white/8 py-3 md:block xl:hidden">
          ...
          <div className="mt-3 border-t border-white/8 pt-3"><WalletStatus compact /></div>
        </div>}
      </div>
    </header>
