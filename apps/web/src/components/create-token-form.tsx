...
export function CreateTokenForm({ authenticated, chainId, walletAddress, walletMode = "embedded", execute, onSuccess }: Props) {
  ...
  return <section className="panel mt-8 max-w-3xl" aria-label="Create token form">
    ...
    {!review ? <div className="grid gap-5">
      ...
      <div className="rounded-xl border border-violet-400/15 bg-violet-400/[0.035] p-4"><p className="text-sm font-medium text-violet-200">Permanent supply settings</p><dl className="mt-3 grid gap-3 text-sm sm:grid-cols-3"><Summary label="Fixed supply" value="1,000,000,000" /><Summary label="Creator allocation" value="0" /><Summary label="Curve inventory" value="Full allocation" /></dl><p className="mt-3 text-xs leading-5 text-zinc-500">All inventory is assigned atomically to the bonding curve. These settings cannot be edited in this form.</p></div>
      ...
      <div className="rounded-xl border border-white/8 bg-black/15 p-4"><dl className="grid gap-4 text-sm sm:grid-cols-3"><Summary label="Fixed supply" value="1,000,000,000" /><Summary label="Creator allocation" value="0" /><Summary label="Initial purchase" value="None" /></dl></div>
