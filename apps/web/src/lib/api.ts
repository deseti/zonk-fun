import type { ActivityPage, ApiError, ChartPage, CreatorProfile, Pricing, Token, TokenPage, TradePage } from "@zonk/types";
import { z } from "zod";
...
export const api = {
  ...
  pricing: (address: string) => request<Pricing>(`/api/v1/tokens/${encodeURIComponent(address)}/pricing`, z.object({ token_address: z.string(), current_price: z.string().nullable(), fully_diluted_value: z.string().nullable(), source: z.literal("indexed_v3_curve") })),
  chart: (address: string, query = "") => request<ChartPage>(`/api/v1/tokens/${encodeURIComponent(address)}/chart${query}`, z.object({ items: z.array(z.object({ bucket_start: z.number(), trade_count: z.number(), buy_count: z.number(), sell_count: z.number(), volume: z.string(), unique_trader_count: z.number(), open_price: z.string().nullable(), high_price: z.string().nullable(), low_price: z.string().nullable(), close_price: z.string().nullable() })) })),
...
export type { ActivityPage, ApiError, ChartPage, CreatorProfile, Pricing, Token, TokenPage, TradePage };
