...
export default function TokenDetailPage() {
  ...
  return <main className="container flex-1 py-12"><p className="eyebrow">Token detail</p><div className="mt-3 flex flex-wrap items-start justify-between gap-4"><div><h1 className="text-3xl font-semibold text-white">{token.name}</h1><p className="mt-2 text-cyan-300">{token.symbol}</p></div><code className="text-xs text-zinc-500">{token.address}</code></div><div className="mt-8 grid gap-4 sm:grid-cols-3"><div className="panel"><p className="text-sm text-zinc-500">Indexed volume</p><p className="mt-2 text-xl text-white">{token.metrics.volume}</p></div><div className="panel"><p className="text-sm text-zinc-500">Trades</p><p className="mt-2 text-xl text-white">{token.metrics.trade_count}</p></div><div className="panel"><p className="text-sm text-zinc-500">Onchain read</p><p className="mt-2 text-xl text-white">{onchain ?? "Checking…"}</p></div></div><section className="mt-10 panel"><h2 className="text-lg font-semibold text-white">Canonical indexed state</h2><dl className="mt-5 grid gap-4 text-sm sm:grid-cols-2"><div><dt className="text-zinc-500">Creator</dt><dd className="mt-1 break-all text-zinc-200">{token.creator}</dd></div><div><dt className="text-zinc-500">Created at block</dt><dd className="mt-1 text-zinc-200">{token.created_at.block_number}</dd></div><div><dt className="text-zinc-500">Market cap</dt><dd className="mt-1 text-zinc-200">{token.metrics.market_cap ?? "Not indexed"}</dd></div><div><dt className="text-zinc-500">Holder count</dt><dd className="mt-1 text-zinc-200">{token.metrics.holder_count ?? "Not indexed"}</dd></div></dl></section></main>;
}
